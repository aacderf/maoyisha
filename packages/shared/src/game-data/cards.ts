...
export const EQUIPMENT_VARIANTS: Partial<
  Record<CardKey, Array<{ name: string; equipmentKey: EquipmentKey; range?: number }>>
> = {
  weapon: [
...
export function cardDef(key: CardKey): Omit<GameCard, "id" | "suit" | "rank"> {
  const defs: Record<CardKey, Omit<GameCard, "id" | "suit" | "rank">> = {
    sha: {
      name: "杀",
      type: "sha",
      cardKey: "sha",
...
export function createStarterDeck(): GameCard[] {
  ...
  const deckPlan: Array<[CardKey, number]> = [
    ["sha", 30],
    ["fire_sha", 5],
    ["thunder_sha", 4],
    ["shan", 22],
  ...
  const occurrences: Partial<Record<CardKey, number>> = {};
...
function decorateDeckCard(card: GameCard, occurrence: number): GameCard {
