...
// Deterministic rules boundary: optional effects become explicit choices; only locked/passive effects resolve automatically.
import type {
  CardArea,
  CardKey,
  CardResponseType,
...
const IDENTITY_DISTRIBUTION: Record<number, IdentityRole[]> = {
...
function normalizeGameMode(mode: GameMode | undefined, playerCount: number): GameMode {
  if (mode) {
    if (mode === "team2v2" && playerCount !== 4) throw new Error("2V2 仅支持 4 名玩家。");
    if (mode === "identity" && playerCount < 5) throw new Error("身份局至少需要 5 名玩家。");
    return mode;
  }
  if (playerCount === 4) return "team2v2";
  return playerCount >= 5 ? "identity" : "free";
}

function assignIdentityRoles(mode: GameMode, playerCount: number, seed: string): Array<IdentityRole | undefined> {
  if (mode !== "identity") return Array.from({ length: playerCount }, () => undefined);
  const roles = IDENTITY_DISTRIBUTION[playerCount];
  if (!roles) throw new Error("身份局仅支持 5-8 人。");
  return shuffleItems(roles, `${seed}-identity`);
}

function assignTeamId(mode: GameMode, index: number): TeamId | undefined {
  if (mode !== "team2v2") return undefined;
...
export function createGame(input: CreateGameInput): GameState {
  ...
  const seats: SeatState[] = input.players.map((player, index) => {
    const character =
      approvedCharacters.find((item) => item.id === player.characterId) ??
  ...
  for (const seat of seats) drawCards(state, seat, 4);
...
export function applyGameAction(state: GameState, action: GameAction): GameActionResult {
  ...
  if (next.pendingChoice) {
    const choicePlayerId =
      action.type === "AUTO_TIMEOUT" ? requireSeatById(next, next.pendingChoice.chooserSeatId).playerId : action.playerId;
  ...
  if (next.pendingResponse) {
    if (action.type === "RESPOND_CARD") return finalizeAction(respondCard(next, action.playerId, action.cardId));
    ...
    if (action.type === "PASS_RESPONSE" || action.type === "AUTO_TIMEOUT") {
      ...
      const responsePlayerId =
        action.type === "AUTO_TIMEOUT" ? requireSeatById(next, next.pendingResponse.responderSeatId).playerId : action.playerId;
  ...
  if (current.playerId !== action.playerId) throw new Error("还没轮到你行动。");
  if (!current.alive) throw new Error("已出局玩家不能行动。");
...
export function getCurrentSeat(state: GameState): SeatState {
  const seat = state.seats[state.currentSeatIndex];
  if (!seat) throw new Error("当前座位不存在。");
  return seat;
}

export function getHandLimit(seat: SeatState): number {
  let limit = seat.character.id === "builtin-yan-laoban" ? 3 : Math.max(1, seat.hp);
  if (isDengGou(seat) && seat.skillState?.dengRenegadeLimitBoost) limit += 2;
  if (isHuangDaxian(seat) && seat.skillState?.huangHandLimitBonus) limit += 3;
  return limit;
}

function startDengOpeningChoice(state: GameState, events: string[], queue = collectDengOpeningQueue(state)): boolean {
  if (state.gameMode !== "identity") return false;
...
function collectDengOpeningQueue(state: GameState): string[] {
  if (state.gameMode !== "identity") return [];
  return state.seats
    .filter(
      (seat) =>
        seat.alive &&
...
function finishDengOpeningChoice(state: GameState, events: string[]): GameActionResult {
...
function playCard(
  ...
  if (state.phase !== "play") throw new Error("当前阶段不能出牌。");

  const card = takeCard(current.hand, cardId);
  if (card) markCardVoice(state, current, card, { targetSeatId, targetSeatIds });
  ...
  try {
    ...
    if (card.cardKey === "tao") {
      if (current.hp >= current.maxHp) throw new Error("体力已满，不能使用桃。");
    ...
    if (card.cardKey === "jiu") {
      if (state.activeTurn.jiuUsed) throw new Error("本回合已经使用过酒。");
      discardUsedCard(state, current, card);
...
function playFangtianSha(
  ...
  if (targetSeatIds.length < 1 || targetSeatIds.length > 3) throw new Error("方天画戟最多指定三名目标。");
  ...
  if (state.usedShaThisTurn) throw new Error("本回合已经使用过杀。");
  discardUsedCard(state, current, card);
  ...
  const effect: Extract<PendingEffect, { kind: "aoe" }> = {
    kind: "aoe",
...
function playSha(
  ...
  events: string[],
  discardPhysicalCard = true
): void {
  if (isHuangDaxian(current) && current.skillState?.huangNoSha) throw new Error("黄大仙本出牌阶段不能使用杀。");
  const target = requireReachableTarget(state, current, targetSeatId, card);
  if (isHongAccompliceBlocked(state, current, target)) throw new Error("本轮共犯不能对虹吸量使用杀。");
  const hasZhuge = hasWeaponKey(current, "zhuge");
  if (state.usedShaThisTurn && hasZhuge) state.usedShaThisTurn = false;
  const shaUsedCount = Number(current.skillState?.shaUsedCount ?? 0);
  ...
  if (shaUsedCount < shaLimit) state.usedShaThisTurn = false;
  if (state.usedShaThisTurn) throw new Error("本回合已经使用过杀。");
  if (discardPhysicalCard) discardUsedCard(state, current, card);
  current.skillState = { ...current.skillState, shaUsedCount: shaUsedCount + 1 };
  if (!hasZhuge && shaUsedCount + 1 >= shaLimit) state.usedShaThisTurn = true;
  state.activeTurn.shaUsed = true;
  state.activeTurn.firstShaPlayed = true;
  ...
  const damageNature =
    card.cardKey === "sha" && hasWeaponKey(current, "zhuque") ? "fire" : (card.damageNature ?? "normal");
  state.activeTurn.jiuDamageBonus = 0;
  const effect: Extract<PendingEffect, { kind: "damage" }> = {
    kind: "damage",
    sourceSeatId: current.seatId,
  ...
  if (dengBonus) events.push(`${current.playerName} 的三五使本次杀伤害 +1。`);
  if (damageNature === "fire" && card.cardKey === "sha") events.push(`${current.playerName} 的朱雀羽扇将普通杀转化为火杀。`);
  if (
    hasWeaponKey(current, "cixiong") &&
  ...
  if (startDengShaTransferChoice(state, current, target, card, effect, events)) return;
  startShaResponse(state, current, target, card, effect, events);
}

function startShaResponse(
  state: GameState,
  source: SeatState,
  target: SeatState,
  card: GameCard,
  effect: Extract<PendingEffect, { kind: "damage" }>,
  events: string[]
): void {
...
function startDengShaTransferChoice(
  state: GameState,
  source: SeatState,
  target: SeatState,
  card: GameCard,
  ...
  if (!isDengGou(target)) return false;
  ...
  const targetSeatIds = state.seats
    ...
    .map((seat) => seat.seatId);
  if (targetSeatIds.length === 0) return false;
...
function discardCard(state: GameState, current: SeatState, cardId: string): GameActionResult {
  if (state.phase !== "discard") throw new Error("当前阶段不能弃牌。");
  if (current.hand.length <= getHandLimit(current)) throw new Error("当前不需要继续弃牌。");
...
function chooseOpeningIdentity(state: GameState, playerId: string, reveal: boolean): GameActionResult {
...
function advancePhaseAction(state: GameState, current: SeatState, endTurnRequested: boolean): GameActionResult {
...
function endTurn(state: GameState, current: SeatState): GameActionResult {
...
function autoTimeout(state: GameState, current: SeatState): GameActionResult {
...
function respondCard(state: GameState, playerId: string, cardId: string): GameActionResult {
  ...
  if (card) markCardVoice(state, responder, card);
  if (!card) throw new Error("响应牌不存在。");

  const dyingSeat = state.pendingDying ? requireSeatById(state, state.pendingDying.seatId) : undefined;
  const canUseJiuAsRescue =
    pending.responseType === "tao" &&
    card.cardKey === "jiu" &&
  ...
  if (pending.effect.kind === "duel") {
    const nextResponder =
      responder.seatId === pending.effect.sourceSeatId
...
function respondBaoDoubleWuxie(state: GameState, action: Extract<GameAction, { type: "USE_SKILL" }>): GameActionResult {
  const pending = requirePendingResponse(state, action.playerId);
  if (pending.responseType !== "wuxie" || pending.mode !== "global") throw new Error("当前不能视为使用无懈可击。");
  const responder = requireResponseSeatForPlayer(state, pending, action.playerId);
  if (!isBaoTaihou(responder)) throw new Error("只有包太后可以发动愚蠢。");
  const ids = Array.from(new Set(action.cardIds ?? []));
  if (ids.length !== 2) throw new Error("请选择两张手牌视为无懈可击。");
  const cards = ids.map((id) => takeCard(responder.hand, id));
  if (cards.some((card) => !card)) {
    for (const card of cards) if (card) responder.hand.push(card);
    throw new Error("选择的手牌不存在。");
  }
  for (const card of cards as GameCard[]) discardToPile(state, responder, card);
  markSkillVoice(state, responder, action);
  const events = [`${responder.playerName} 发动愚蠢，将两张手牌当无懈可击使用。`];
  return continueWuxieResponse(state, pending, responder, events);
}

function continueWuxieResponse(
  state: GameState,
...
function startPostShanWeaponChoice(
  state: GameState,
  effect: Extract<PendingEffect, { kind: "damage" }>,
  events: string[],
  skipQinglong = false
...
function resumeShaResponseFromDamage(
  state: GameState,
  effect: Extract<PendingEffect, { kind: "damage" }>,
  events: string[]
): void {
...
function passResponse(state: GameState, playerId: string): GameActionResult {
  const pending = requirePendingResponse(state, playerId);
  const responder = requireResponseSeatForPlayer(state, pending, playerId);
  ...
  if (pending.responseType === "shan" && pending.effect.kind === "damage") {
    clearPendingResponse(state);
    const paused = applyDamage(
      state,
      pending.effect.sourceSeatId,
      pending.effect.targetSeatId,
      pending.effect.amount,
      pending.effect.cardName,
      events,
      pending.effect.nature,
      undefined,
      pending.effect.ignoreArmor,
      { cardKey: pending.effect.sourceCardKey, suit: pending.effect.sourceSuit }
    );
    if (!paused) state.logs.push(...events.slice(1));
    return { state, events };
  }
  ...
  if (pending.effect.kind === "duel") {
    ...
    if (!state.pendingResponse && state.phase !== "finished") state.phase = "play";
    state.logs.push(...events.slice(1));
    return { state, events };
  }
...
function timeoutGlobalWuxie(state: GameState): GameActionResult {
  const pending = state.pendingResponse;
  if (!pending || pending.responseType !== "wuxie" || pending.mode !== "global") throw new Error("当前不是全场无懈响应。");
  const events = ["无懈可击响应窗口结束，未响应者视为不响应。"];
  clearPendingResponse(state);
  return finalizeWuxieResponse(state, pending, events);
}

function buildTrickEffect(
  state: GameState,
  source: SeatState,
  card: GameCard,
  targetSeatId?: string
  ...
  if (card.cardKey === "shunshou") {
    ...
    if (getSeatDistance(state, source, target) > 1) throw new Error("顺手牵羊只能选择距离 1 内的目标。");
  ...
  if (card.cardKey === "jiedao") {
    const target = requireTarget(state, source, targetSeatId);
    if (!target.equipment.weapon) throw new Error("借刀杀人的目标必须装备武器。");
...
function startWuxieCheck(
  ...
  const eligible = eligibleWuxieResponders(state);
  if (eligible.length === 0) return resolveEffect(state, effect, events);
...
function startTiesuoChoice(
  state: GameState,
  source: SeatState,
  card: GameCard,
  initialTargetSeatId: string | undefined,
  events: string[]
): GameActionResult {
...
function resolveEffect(state: GameState, effect: PendingEffect, events: string[]): GameActionResult {
  if (effect.kind === "draw") {
    const source = requireSeatById(state, effect.sourceSeatId);
  ...
  if (effect.kind === "discard-random" || effect.kind === "steal-random") {
    const normalized: Extract<PendingEffect, { kind: "target-card" }> = {
      kind: "target-card",
  ...
  if (effect.kind === "heal-all") {
    for (const seat of state.seats.filter((item) => item.alive)) {
      if (seat.hp < seat.maxHp) seat.hp = Math.min(seat.maxHp, seat.hp + effect.amount);
    }
...
function startTargetCardChoice(
  state: GameState,
  effect: Extract<PendingEffect, { kind: "target-card" }>,
  events: string[]
): GameActionResult {
  const source = requireSeatById(state, effect.sourceSeatId);
  const target = requireSeatById(state, effect.targetSeatId);
...
function startJiedaoChoice(
  state: GameState,
  effect: Extract<PendingEffect, { kind: "jiedao" }>,
  events: string[]
): GameActionResult {
  const source = requireSeatById(state, effect.sourceSeatId);
...
function startHuogongChoice(
  state: GameState,
  effect: Extract<PendingEffect, { kind: "huogong" }>,
  events: string[]
): GameActionResult {
  const source = requireSeatById(state, effect.sourceSeatId);
  const target = requireSeatById(state, effect.targetSeatId);
...
function choosePendingCard(
  ...
  if (pending.cardName === "shen-boss-prevent-damage" && pending.effect.kind === "damage") {
    ...
    if (!discarded) throw new Error("减伤弃置牌不存在。");
  ...
  if (pending.cardName === "cixiong-sword" && pending.effect.kind === "damage") {
    const choice = requireChoiceOption(pending, cardId, choiceIdValue);
    const discarded = takeChoiceCard(state, choice);
    if (!discarded) throw new Error("雌雄双股剑弃置牌不存在。");
    discardToPile(state, chooser, discarded);
    state.pendingChoice = undefined;
  ...
  if (pending.cardName === "qinglong-blade" && pending.effect.kind === "damage") {
    ...
    if (!sha || !isSha(sha)) throw new Error("请选择一张杀继续追击。");
  ...
  if (pending.cardName === "shen-boss-prevent-damage" && pending.effect.kind === "damage") {
    ...
    const paused = applyDamage(
      ...
      undefined,
      effect.ignoreArmor,
      { cardKey: effect.sourceCardKey, suit: effect.sourceSuit },
      Boolean(effect.skipKgMercy),
      Boolean(effect.skipHanbingChoice),
      true
    );
    if (!paused) state.logs.push(...events);
    return { state, events };
  }
  ...
  if (pending.kind === "huogong-reveal" && pending.effect.kind === "huogong") {
    ...
    if (!shown) throw new Error("火攻展示牌不存在。");
  ...
  if (pending.kind === "public-card" && pending.effect.kind === "wugu") {
    ...
    if (!card) throw new Error("公共牌不存在。");
...
function choosePendingCards(state: GameState, playerId: string, cardIds: string[]): GameActionResult {
  const pending = requirePendingChoice(state, playerId);
  const chooser = requireSeatById(state, pending.chooserSeatId);
  if (pending.kind === "multi-card" && pending.cardName === "tudou-shenggen-prevent") {
    const uniqueIds = Array.from(new Set(cardIds));
    if (uniqueIds.length !== 2) throw new Error("生根需要弃置两张手牌来防止传导伤害。");
    ...
    for (const card of discarded) discardToPile(state, chooser, card);
    const effect = pending.effect;
  ...
  if (pending.kind === "multi-card" && pending.cardName === "shen-boss-cycle") {
    const uniqueIds = Array.from(new Set(cardIds));
    if (uniqueIds.length < 1 || uniqueIds.length > 2) throw new Error("请选择一至两张手牌。");
    const discarded: GameCard[] = [];
    for (const id of uniqueIds) {
      const card = takeCard(chooser.hand, id);
      if (!card) throw new Error("选择的手牌不存在。");
      discarded.push(card);
    }
    for (const card of discarded) discardToPile(state, chooser, card);
  ...
  const uniqueIds = Array.from(new Set(cardIds));
  if (uniqueIds.length !== 2) throw new Error("贯石斧需要弃置两张牌。");
  const selected = uniqueIds.map((id) => pending.choices.find((choice) => choice.cardId === id));
  if (selected.some((choice) => !choice)) throw new Error("贯石斧选择的牌不合法。");
  const removed: GameCard[] = [];
  for (const choice of selected as ChoiceCardOption[]) {
    const card = takeChoiceCard(state, choice);
    if (!card) throw new Error("贯石斧选择的牌不存在。");
    removed.push(card);
  }
  for (const card of removed) discardToPile(state, chooser, card);
  state.pendingChoice = undefined;
...
function choosePendingTarget(state: GameState, playerId: string, targetSeatId: string): GameActionResult {
  const pending = requirePendingChoice(state, playerId);
  const chooser = requireSeatById(state, pending.chooserSeatId);
  if (pending.kind === "skill-target" && pending.cardName === "tudou-faya") {
    if (!isTudou(chooser)) throw new Error("只有土豆可以发动发芽。");
    if (!pending.targetSeatIds?.includes(targetSeatId)) throw new Error("目标不合法。");
    const target = requireSeatById(state, targetSeatId);
  ...
  if (pending.kind === "sha-transfer" && pending.effect.kind === "damage") {
    ...
    if (charges <= 0) throw new Error("转杀次数不足。");
...
function choosePendingTargets(state: GameState, playerId: string, targetSeatIds: string[]): GameActionResult {
  const pending = requirePendingChoice(state, playerId);
  const chooser = requireSeatById(state, pending.chooserSeatId);
  ...
  for (const seatId of uniqueTargets) {
    ...
    if (!target.alive) throw new Error("目标不合法。");
  }

  state.pendingChoice = undefined;
  const effect: Extract<PendingEffect, { kind: "chain" }> = {
    ...pending.effect,
...
function choosePendingSkill(state: GameState, action: Extract<GameAction, { type: "USE_SKILL" }>): GameActionResult {
  const pending = requirePendingChoice(state, action.playerId);
  const chooser = requireSeatById(state, pending.chooserSeatId);
  if (pending.kind !== "skill-confirm") throw new Error("当前不需要确认技能。");
  ...
  if (action.skillId === "sanshui-kg-mercy") {
    if (!isSanshui(chooser)) throw new Error("只有三水先生可以发动 kg 的怜悯。");
    if (pending.effect.kind !== "damage") throw new Error("kg 的怜悯缺少伤害结算信息。");
    const target = requireSeatById(state, pending.effect.targetSeatId);
    if (target.skillState?.kgSourceSeatId !== chooser.seatId) throw new Error("目标没有对应的 kg 标记。");
    const discarded = takeDeterministicHandCard(state, target, `kg-mercy-${state.turn}-${target.hand.length}`);
    if (discarded) discardToPile(state, target, discarded);
    state.pendingChoice = undefined;
    if (state.phase !== "finished") state.phase = "play";
    const events = [
      discarded
  ...
  if (action.skillId === "hanbing-sword") {
    if (pending.effect.kind !== "damage") throw new Error("寒冰剑缺少伤害结算信息。");
    const target = requireSeatById(state, pending.effect.targetSeatId);
  ...
  if (action.skillId === "bagua-armor") {
    if (pending.effect.kind !== "damage") throw new Error("八卦阵缺少响应信息。");
    state.pendingChoice = undefined;
    const events: string[] = [];
  ...
  if (action.skillId === "shen-black-sha-steal") {
    if (!isShenZhuxi(chooser) || pending.effect.kind !== "steal-random") throw new Error("乒乓高手结算信息无效。");
    const target = requireSeatById(state, pending.effect.targetSeatId);
    const stolen = takeDeterministicHandCard(state, target, `shen-${chooser.seatId}-${target.seatId}-${state.turn}`);
    if (stolen) chooser.hand.push(stolen);
    chooser.skillState = {
      ...chooser.skillState,
  ...
  if (action.skillId === "sanshui-kg-trick-draw") {
    if (!isSanshui(chooser) || pending.effect.kind !== "draw") throw new Error("op的神罚摸牌结算信息无效。");
    const round = getRoundNumber(state);
    drawCards(state, chooser, 1);
  ...
  if (action.skillId === "yang-qiaoshe") {
    if (!isYangHaiyan(chooser)) throw new Error("只有杨嗨厌可以发动巧舌如簧。");
    const round = getRoundNumber(state);
  ...
  if (action.skillId === "sanshui-zixin") {
    ...
    if (chooser.skillState?.sanshuiZixinUsed || chooser.maxHp <= 1) throw new Error("自刎归天已使用或体力上限不足。");
    for (const card of chooser.hand.splice(0)) discardToPile(state, chooser, card);
  ...
  if (action.skillId === "shen-boss-talent-judge") {
    if (!isShenLaoban(chooser)) throw new Error("只有天之骄子·沈老板可以发动腰折的天才。");
    const judge = drawJudgeCard(state);
    state.discardPile.push(judge);
    state.revealedCards.push(judge);
  ...
  if (action.skillId === "gay-chenmo" || pending.cardName === "沉默") {
    if (!isGayGuan(chooser)) throw new Error("只有 gay管可以发动沉默。");
    if (chooser.maxHp <= 1) throw new Error("体力上限不足，不能发动沉默。");
    const round = getRoundNumber(state);
    if (Number(chooser.skillState?.gaySilentUsedRound ?? -1) === round) throw new Error("沉默本轮已经使用过。");
    chooser.maxHp = Math.max(1, chooser.maxHp - 1);
  ...
  if (action.skillId === "gay-weixiao" || pending.cardName === "微笑") {
    if (!isGayGuan(chooser)) throw new Error("只有 gay管可以发动微笑。");
    if (chooser.maxHp >= 6) throw new Error("体力上限已达到 6。");
...
function passPendingChoice(state: GameState, playerId: string): GameActionResult {
  const pending = requirePendingChoice(state, playerId);
  const chooser = requireSeatById(state, pending.chooserSeatId);
  ...
  if (pending.kind === "skill-confirm" && pending.cardName === "sanshui-kg-mercy" && pending.effect.kind === "damage") {
    ...
    const paused = applyDamage(
      ...
      { cardKey: pending.effect.sourceCardKey, suit: pending.effect.sourceSuit },
      true,
      Boolean(pending.effect.skipHanbingChoice)
    );
    if (!paused) state.logs.push(...events);
    return { state, events };
  }
  ...
  if (pending.kind === "skill-confirm" && pending.cardName === "hanbing-sword" && pending.effect.kind === "damage") {
    ...
    const paused = applyDamage(
      ...
      pending.effect.ignoreArmor,
      { cardKey: pending.effect.sourceCardKey, suit: pending.effect.sourceSuit },
      Boolean(pending.effect.skipKgMercy),
      true
    );
    if (!paused) state.logs.push(...events);
    return { state, events };
...
function continueWuguChoice(
  state: GameState,
  queue: string[],
  effect: Extract<PendingEffect, { kind: "wugu" }>,
  events: string[]
): GameActionResult {
...
function continueAoe(state: GameState, effect: Extract<PendingEffect, { kind: "aoe" }>, events: string[]): GameActionResult {
...
function nextAoeResponse(
  state: GameState,
  effect: Extract<PendingEffect, { kind: "aoe" }>
...
function applyDamage(
  ...
  ignoreArmor = false,
  sourceCard?: { cardKey?: CardKey; suit?: CardSuit },
  skipKgMercy = false,
  skipHanbingChoice = false,
  skipShenBossPrevent = false
): boolean {
  ...
  if (!ignoreArmor && hasArmorKey(target, "baiyin") && amount > 1) amount = 1;
  if (!ignoreArmor && hasArmorKey(target, "tengjia") && nature === "fire") amount += 1;
  const chainedTargets =
    nature !== "normal" && target.chained
      ? state.seats.filter((seat) => seat.alive && seat.chained && seat.seatId !== target.seatId).map((seat) => seat.seatId)
      : [];

  if (nature !== "normal") target.chained = false;
  ...
  if (amount > 0 && triggerCjjPoison(state, source, target, cardName, events)) return true;

  if (amount > 0 && triggerTudouRootDamage(state, source, target, amount, cardName, nature, sourceCard, events)) return true;
  ...
  if (
    ...
    const horseChoices = (["attackHorse", "defenseHorse"] as EquipmentSlot[])
      .map((slot) => {
        const horse = target.equipment[slot];
        return horse ? choiceFromCard(horse, "equipment", target.seatId, slot) : undefined;
      })
      .filter((choice): choice is ChoiceCardOption => Boolean(choice));
  ...
  if (state.phase !== "finished" && !state.pendingChoice && !state.pendingResponse) state.phase = "play";
  return false;
}

function triggerCjjPowderDraw(
  state: GameState,
  source: SeatState,
  target: SeatState,
  sourceCard: { cardKey?: CardKey; suit?: CardSuit } | undefined,
  cardName: string,
  events: string[]
): boolean {
  if (!isCjj(target) || source.seatId === target.seatId || source.character.faction !== "shu") return false;
  if (!isTrickDamage(sourceCard, cardName)) return false;
  const round = getRoundNumber(state);
...
function triggerCjjPoison(
  ...
  if (source.seatId === target.seatId || cardName === "试管") return false;
  ...
  if (typeof poisonSourceSeatId !== "string" || state.turn > expireTurn) return false;
  source.skillState = {
    ...source.skillState,
  ...
  if (poisonSource) events.push(`毒标记来自 ${poisonSource.playerName}，现已移除。`);
  if (source.hp <= 0) return startDying(state, source, target.seatId, "试管", events);
  return false;
}

function triggerTudouRootDamage(
  state: GameState,
  source: SeatState,
  damaged: SeatState,
  amount: number,
  ...
  if (cardName === "生根" || state.pendingChoice || state.pendingResponse || state.pendingDying) return false;
  const link = findTudouRootLink(state, damaged);
  if (!link || !link.counterpart.alive || link.counterpart.seatId === damaged.seatId) return false;
  const effect: Extract<PendingEffect, { kind: "damage" }> = {
    kind: "damage",
    sourceSeatId: source.seatId,
...
function findTudouRootLink(state: GameState, damaged: SeatState): { owner: SeatState; counterpart: SeatState } | undefined {
  const round = getRoundNumber(state);
  if (isTudou(damaged) && Number(damaged.skillState?.tudouRootRound ?? -1) === round) {
    ...
    if (counterpart?.alive) return { owner: damaged, counterpart };
  }
  const ownerSeatId = damaged.skillState?.tudouRootSourceSeatId;
  if (typeof ownerSeatId !== "string" || Number(damaged.skillState?.tudouRootRound ?? -1) !== round) return undefined;
...
function isTrickDamage(sourceCard: { cardKey?: CardKey; suit?: CardSuit } | undefined, cardName: string): boolean {
  if (sourceCard?.cardKey && cardDef(sourceCard.cardKey).category === "trick") return true;
...
function continueDyingFromPending(state: GameState, events: string[]): boolean {
  ...
  if (!first) {
    ...
    if (source && source.seatId !== target.seatId) events.push(`${source.playerName} 击败 ${target.playerName}。`);
    target.alive = false;
    target.hp = 0;
...
function resumeAfterDyingRecovery(state: GameState): void {
  const resume = state.pendingDying?.resume;
  state.pendingDying = undefined;
...
function finishIfOnlyOneAlive(state: GameState, events: string[]): void {
  if (state.gameMode === "identity" && finishIdentityGameIfNeeded(state, events)) return;
  if (state.gameMode === "team2v2" && finishTeamGameIfNeeded(state, events)) return;
...
function finishTeamGameIfNeeded(state: GameState, events: string[]): boolean {
  const aliveTeams = new Set(state.seats.filter((seat) => seat.alive && seat.teamId).map((seat) => seat.teamId as TeamId));
...
function handleSeatDeath(state: GameState, dead: SeatState, sourceSeatId: string | undefined, events: string[]): void {
  ...
  if (state.gameMode !== "identity" || !sourceSeatId) return;
  const killer = state.seats.find((seat) => seat.seatId === sourceSeatId);
  if (!killer || killer.seatId === dead.seatId) return;
...
function finishIdentityGameIfNeeded(state: GameState, events: string[]): boolean {
...
function revealAllIdentities(state: GameState): void {
  for (const seat of state.seats) seat.identityRevealed = true;
}

function discardAllCards(state: GameState, seat: SeatState): void {
  for (const card of seat.hand.splice(0)) discardToPile(state, seat, card);
  for (const slot of Object.keys(seat.equipment) as EquipmentSlot[]) {
    removeEquipmentCard(state, seat, slot, []);
  }
}

function startPrepareSkillChoice(state: GameState, seat: SeatState, events: string[]): boolean {
  if (!isSanshui(seat) || seat.skillState?.sanshuiKgPreparedTurn === state.turn) return false;
...
function startSkillConfirmChoice(
  state: GameState,
  seat: SeatState,
  skillId: string,
  skillName: string,
  ...
  if (state.pendingChoice || state.pendingResponse || state.pendingDying || state.phase === "finished") return false;
  state.pendingChoice = {
    id: choiceId(state),
    kind: "skill-confirm",
    chooserSeatId: seat.seatId,
...
function skillIdFromConfirmChoice(choice: PendingChoice): string {
  return choice.cardName;
}

function startPlayPhaseSkillChoice(state: GameState, seat: SeatState, events: string[]): boolean {
  if (!isShenLaoban(seat) || seat.skillState?.shenBossTalentCheckedTurn === state.turn) return false;
...
function startDrawPhaseSkillChoice(state: GameState, seat: SeatState, events: string[]): boolean {
  if (seat.skipDrawPhase || isHuangDaxian(seat)) return false;
  const round = getRoundNumber(state);
...
function advancePhase(state: GameState, endTurnRequested = false): string[] {
  const current = getCurrentSeat(state);
  const events: string[] = [];

  if (state.phase === "prepare") {
    if (startPrepareSkillChoice(state, current, events)) return events;
    state.phase = "judge";
  ...
  if (state.phase === "judge") {
    ...
    if (state.pendingResponse || (state.phase as GameState["phase"]) === "finished") return events;
    state.phase = "draw";
    events.push(`${current.playerName} 进入摸牌阶段。`);
  }

  if (state.phase === "draw") {
    if (startDrawPhaseSkillChoice(state, current, events)) return events;
    resolveDrawPhase(state, current, events);
    if ((state.phase as GameState["phase"]) !== "finished") state.phase = current.skipPlayPhase ? "discard" : "play";
    events.push(current.skipPlayPhase ? `${current.playerName} 跳过出牌阶段。` : `${current.playerName} 进入出牌阶段。`);
    if (state.phase === "play" && startPlayPhaseSkillChoice(state, current, events)) return events;
    return events;
  }
  ...
  if (state.phase === "discard") {
    if (current.hand.length > getHandLimit(current)) throw new Error(`请弃牌至 ${getHandLimit(current)} 张。`);
    state.phase = "finish";
    events.push(`${current.playerName} 进入结束阶段。`);
    if (endTurnRequested) {
      resolveFinishPhaseSkills(state, current, events);
...
function resolveFinishPhaseSkills(state: GameState, current: SeatState, events: string[]): void {
  const round = getRoundNumber(state);
  if (
    current.skillState?.hongAccompliceTargetSeatId &&
  ...
  if (
    current.skillState?.hongAccompliceSourceSeatId &&
    Number(current.skillState?.hongAccompliceRound ?? -1) === round
  ) {
    drawCards(state, current, 1);
...
function updateDengTransferProgress(state: GameState, events: string[]): void {
  const aliveCount = Math.max(1, state.seats.filter((seat) => seat.alive).length);
  const threshold = aliveCount * 2;
  for (const seat of state.seats) {
    if (!seat.alive || !isDengGou(seat)) continue;
    ...
    if (progress >= threshold) {
      ...
      if (charges > 0) events.push(`${seat.playerName} 的转杀机会已准备。`);
    } else {
      seat.skillState = {
        ...seat.skillState,
  ...
  for (const seat of state.seats) {
    if (!seat.alive || !isBaoTaihou(seat)) continue;
    ...
    if (progress >= threshold) {
      ...
      if (Number(seat.skillState?.baoPanicCharges ?? 0) > 0) events.push(`${seat.playerName} 的恐慌已准备。`);
    } else {
...
function resetTurnSkillState(seat: SeatState): void {
  seat.skillState = {
    ...seat.skillState,
    yanWuguUsed: false,
...
function clearTurnScopedSkillState(state: GameState): void {
  for (const seat of state.seats) {
...
function clearExpiredCharacterMarks(state: GameState, events: string[]): void {
  const round = getRoundNumber(state);
  for (const seat of state.seats) {
...
function resolveHuangJudgePhase(state: GameState, seat: SeatState, events: string[]): void {
  if (!isHuangDaxian(seat) || !seat.alive) return;
  const judge = drawJudgeCard(state);
  state.revealedCards.push(judge);
...
function resolveDrawPhase(state: GameState, seat: SeatState, events: string[]): void {
  ...
  if (panicMinus) seat.skillState = { ...seat.skillState, baoPanicDrawMinus: false };
  ...
  if (hongBonus) events.push(`${seat.playerName} 触发九个千万，额外摸 1 张。`);
  events.push(`${seat.playerName} 摸 ${drawCount} 张牌。`);
  if (dengBonus) events.push(`${seat.playerName} 的三五额外摸 1 张。`);
  if (haijieBonus) events.push(`${seat.playerName} 的美的额外摸 1 张。`);
}

function appendDrawPhaseSkillLogs(seat: SeatState, events: string[], baoBonus: number, panicMinus: number): void {
  if (baoBonus) events.push(`${seat.playerName} 触发忠犬，额外摸 1 张。`);
  if (panicMinus) events.push(`${seat.playerName} 受恐慌影响，少摸 1 张。`);
}

function applyPassiveSkills(state: GameState, events: string[]): void {
  if (state.phase === "opening" || state.phase === "finished" || state.pendingDying) return;

  const current = state.seats[state.currentSeatIndex];
  const round = getRoundNumber(state);
  for (const seat of state.seats) {
    if (!seat.alive || !isYanLaoban(seat) || seat.hand.length >= 3) continue;

    // 弃牌过程中暂停补牌，避免“弃一张、补一张”的循环；离开弃牌阶段后会再次检查。
    if (current?.seatId === seat.seatId && state.phase === "discard") continue;
...
function useCharacterSkill(state: GameState, current: SeatState, action: Extract<GameAction, { type: "USE_SKILL" }>): GameActionResult {
  markSkillVoice(state, current, action);

  if (action.skillId === "zhangba-sha") {
    if (state.phase !== "play" || !hasWeaponKey(current, "zhangba")) throw new Error("当前不能发动丈八蛇矛。");
    const ids = Array.from(new Set(action.cardIds ?? []));
    if (ids.length !== 2) throw new Error("丈八蛇矛需要选择两张手牌。");
    const subcards = ids.map((id) => takeCard(current.hand, id));
    if (subcards.some((card) => !card)) {
      for (const card of subcards) if (card) current.hand.push(card);
      throw new Error("丈八蛇矛选择的手牌不存在。");
    }
    for (const card of subcards as GameCard[]) discardToPile(state, current, card);
    const virtualSha: GameCard = {
  ...
  if (isShenZhuxi(current) && action.skillId === "shen-xuesheng-dang") {
    if (state.phase !== "play") throw new Error("学生党只能在出牌阶段使用。");
    if (current.skillState?.shenStudentPartyUsed) throw new Error("学生党一局只能使用一次。");
  ...
  if (isBaoTaihou(current) && action.skillId === "bao-konghuang") {
    if (state.phase !== "play") throw new Error("恐慌只能在出牌阶段使用。");
    const charges = Number(current.skillState?.baoPanicCharges ?? 0);
    if (charges <= 0) throw new Error("恐慌次数不足。");
  ...
  if (isHuangDaxian(current) && action.skillId === "huang-use-detained") {
    if (state.phase !== "play") throw new Error("错算扣押牌只能在出牌阶段使用。");
    ...
    if (!cardId || index < 0) throw new Error("请选择一张已扣押的错算牌。");
    ...
    if (drawCount === 1) events.push(`${current.playerName} 触发重算，本回合手牌上限 +3。`);
    if (drawCount === 4) events.push(`${current.playerName} 触发重算，本出牌阶段不能使用杀。`);
    state.logs.push(...events);
    return { state, events };
  }

  if (isHaijieDashen(current) && action.skillId === "haijie-jiujing") {
    if (state.phase !== "play") throw new Error("酒精只能在出牌阶段使用。");
    const round = getRoundNumber(state);
    ...
    if (usedCount >= 2) throw new Error("酒精每轮最多使用两次。");
    const cardId = action.cardIds?.[0];
    if (!cardId) throw new Error("请选择一张锦囊牌置换成酒。");
    const trick = takeCard(current.hand, cardId);
    if (!trick) throw new Error("选择的手牌不存在。");
  ...
  if (isHongXiliang(current) && action.skillId === "hong-tanwu") {
    if (state.phase !== "play") throw new Error("贪污只能在出牌阶段使用。");
    if (state.seats.filter((seat) => seat.alive).length < 3) throw new Error("场上至少 3 人时才能发动贪污。");
    const round = getRoundNumber(state);
    const usedRound = Number(current.skillState?.hongTanwuRoundUsed ?? -99);
    if (round - usedRound < 2) throw new Error("贪污每两轮只能使用一次。");
    const target = requireTarget(state, current, action.targetSeatId);
    current.skillState = {
      ...current.skillState,
  ...
  if (isJuHui(current) && action.skillId === "ju-jianjie-tao") {
    ...
    if (marks < 2) throw new Error("简洁需要 2 枚板书标记。");
    current.skillState = { ...current.skillState, juBoardMarks: marks - 2 };
    if (current.hp < current.maxHp) current.hp += 1;
  ...
  if (isJuHui(current) && action.skillId === "ju-jianjie-copy") {
    const marks = Number(current.skillState?.juBoardMarks ?? 0);
    if (marks < 3) throw new Error("复制技能需要 3 枚板书标记。");
    const target = requireTarget(state, current, action.targetSeatId);
    current.skillState = {
      ...current.skillState,
  ...
  if (isYangzhiTao(current) && action.skillId === "yang-shuji") {
    if (state.phase !== "play") throw new Error("书记只能在出牌阶段使用。");
    if (current.skillState?.yangShujiTurn === state.turn) throw new Error("书记每回合只能使用一次。");
    const takeCount = Math.min(2, state.discardPile.length);
    if (takeCount <= 0) throw new Error("弃牌堆没有可获得的牌。");
  ...
  if (isTudou(current) && action.skillId === "tudou-shenggen") {
    if (state.phase !== "play") throw new Error("生根只能在出牌阶段指定目标。");
    const round = getRoundNumber(state);
    const usedRound = Number(current.skillState?.tudouShenggenUsedRound ?? -99);
    if (round - usedRound < 2) throw new Error("生根每两轮只能指定一次。");
    const target = requireTarget(state, current, action.targetSeatId);
  ...
  if (isCjj(current) && action.skillId === "cjj-shiguan") {
    if (state.phase !== "play") throw new Error("试管只能在出牌阶段使用。");
    if (current.skillState?.cjjShiguanTurn === state.turn) throw new Error("试管每回合只能使用一次。");
    const cardId = action.cardIds?.[0];
    if (!cardId) throw new Error("试管需要弃置一张手牌。");
    const discarded = takeCard(current.hand, cardId);
    if (!discarded) throw new Error("试管选择的手牌不存在。");
    const target = requireTarget(state, current, action.targetSeatId);
  ...
  if (isYangHaiyan(current) && (action.skillId === "yang-xiaoli-nanman" || action.skillId === "yang-xiaoli-wanjian")) {
    if (state.phase !== "play") throw new Error("笑里藏刀只能在出牌阶段使用。");
    const cardId = action.cardIds?.[0];
    if (!cardId) throw new Error("请选择桃园结义或五谷丰登。");
    const original = takeCard(current.hand, cardId);
    if (!original) throw new Error("笑里藏刀选择的手牌不存在。");
    if (original.cardKey !== "taoyuan" && original.cardKey !== "wugu") {
      current.hand.push(original);
      throw new Error("笑里藏刀只能转换桃园结义或五谷丰登。");
    }
    const virtualKey: CardKey = action.skillId === "yang-xiaoli-nanman" ? "nanman" : "wanjian";
    const virtual = {
  ...
  if (state.phase !== "play") throw new Error("下知地理只能在出牌阶段使用。");
  if (current.skillState?.yanWuguUsed) throw new Error("下知地理每回合只能使用一次。");
  const cardIds = Array.from(new Set(action.cardIds ?? []));
  if (cardIds.length !== 2) throw new Error("下知地理需要明确选择两张手牌。");
  const discarded: GameCard[] = [];
  ...
  for (const card of discarded) discardToPile(state, current, card);
  current.skillState = { ...current.skillState, yanWuguUsed: true };
  const events = [`${current.playerName} 发动下知地理，弃两张牌视为使用五谷丰登。`];
  const effect: PendingEffect = { kind: "wugu", sourceSeatId: current.seatId, queue: responseQueueFrom(state, current.seatId, true), cardName: "下知地理" };
  return startWuxieCheck(state, current.seatId, "下知地理", effect, events);
}

function tryBaguaDodge(state: GameState, seat: SeatState, events: string[]): boolean {
  const judge = drawJudgeCard(state);
  state.discardPile.push(judge);
  state.revealedCards.push(judge);
...
function isYanLaoban(seat: SeatState): boolean {
  return seat.character.id === "builtin-yan-laoban";
}

function isShenZhuxi(seat: SeatState): boolean {
  return seat.character.id === "builtin-shen-zhuxi";
}

function isShenLaoban(seat: SeatState): boolean {
  return seat.character.id === "builtin-tianzhi-jiaozi-shen-laoban";
}

function isSanshui(seat: SeatState): boolean {
  return seat.character.id === "builtin-sanshui-xiansheng";
}

function isBaoTaihou(seat: SeatState): boolean {
  return seat.character.id === "builtin-bao-taihou";
}

function isWuMao(seat: SeatState): boolean {
  return seat.character.id === "builtin-wu-mao";
}

function isGayGuan(seat: SeatState): boolean {
  return seat.character.id === "builtin-gay-guan";
}

function isHaijieDashen(seat: SeatState): boolean {
  return seat.character.id === "builtin-haijie-dashen";
}

function isHongXiliang(seat: SeatState): boolean {
  return seat.character.id === "builtin-hong-xiliang";
}

function isJuHui(seat: SeatState): boolean {
  return seat.character.id === "builtin-ju-hui";
}

function isYangzhiTao(seat: SeatState): boolean {
  return seat.character.id === "builtin-yangzhi-tao";
}

function isTudou(seat: SeatState): boolean {
  return seat.character.id === "builtin-tudou";
}

function isCjj(seat: SeatState): boolean {
  return seat.character.id === "builtin-cjj";
}

function isYangHaiyan(seat: SeatState): boolean {
  return seat.character.id === "builtin-yang-haiyan";
}

function isDengGou(seat: SeatState): boolean {
  return seat.character.id === "builtin-deng-gou";
}

function isHuangDaxian(seat: SeatState): boolean {
  return seat.character.id === "builtin-huang-daxian";
}

function getHuangDetainedCards(seat: SeatState): GameCard[] {
  const cards = seat.skillState?.huangDetainedCards;
  return Array.isArray(cards) ? [...(cards as GameCard[])] : [];
}
...
function processTurnStartJudgements(state: GameState, seat: SeatState, events: string[]): void {
...
function drawForCurrentPlayer(state: GameState, amount: number): GameState {
  drawCards(state, getCurrentSeat(state), amount);
  return state;
}

function drawCards(state: GameState, seat: SeatState, amount: number): void {
...
function revealPublicCards(state: GameState, amount: number): void {
  state.publicCards = [];
  for (let index = 0; index < amount; index += 1) {
    if (state.deck.length === 0) {
...
function drawJudgeCard(state: GameState): GameCard {
  if (state.deck.length === 0) {
    state.deck = shuffleDeck(state.discardPile.splice(0), `${state.id}-judge-${state.turn}`);
  }
  const card = state.deck.shift();
  if (!card) throw new Error("牌堆为空，无法判定。");
  return card;
}

function createGeneratedCard(cardKey: CardKey, seed: string): GameCard {
  return {
...
function requirePendingResponse(state: GameState, playerId: string): PendingResponse {
  const pending = state.pendingResponse;
  if (!pending) throw new Error("当前没有需要响应的操作。");
  requireResponseSeatForPlayer(state, pending, playerId);
  return pending;
}

function requireResponseSeatForPlayer(state: GameState, pending: PendingResponse, playerId: string): SeatState {
  const seat = state.seats.find((item) => item.playerId === playerId);
  if (!seat) throw new Error("当前座位不存在。");
  if (!isSeatEligibleForResponse(pending, seat.seatId)) throw new Error("当前不是你响应。");
  return seat;
}

function isSeatEligibleForResponse(pending: PendingResponse, seatId: string): boolean {
  if (pending.mode === "global") {
    return Boolean(
...
function eligibleWuxieResponders(state: GameState): string[] {
  return state.seats
    .filter((seat) => seat.alive && (seat.hand.some((card) => card.cardKey === "wuxie") || (isBaoTaihou(seat) && seat.hand.length >= 2)))
    .map((seat) => seat.seatId);
}

function finalizeWuxieResponse(state: GameState, pending: PendingResponse, events: string[]): GameActionResult {
...
function cancelEffectByWuxie(state: GameState, effect: PendingEffect, events: string[]): GameActionResult {
  if (effect.kind === "delayed" && effect.delayedTrickType === "shandian") {
    const card = takeCard(state.discardPile, effect.cardId) ?? takeCard(state.revealedCards, effect.cardId);
    if (card) takeCard(state.revealedCards, effect.cardId);
    const target = requireSeatById(state, effect.targetSeatId);
...
function requirePendingChoice(state: GameState, playerId: string): PendingChoice {
  const pending = state.pendingChoice;
  if (!pending) throw new Error("当前没有需要选择的操作。");
  const chooser = requireSeatById(state, pending.chooserSeatId);
  if (chooser.playerId !== playerId) throw new Error("当前不是你选择。");
  return pending;
}

function clearPendingResponse(state: GameState) {
  state.pendingResponse = undefined;
  if (!state.pendingDying && state.phase !== "finished") state.phase = "play";
}

function shiftNextResponder(pending: PendingResponse): string | undefined {
  return pending.queue?.shift();
}

function skipDeadResponder(state: GameState, pending: PendingResponse): PendingResponse | undefined {
  if (state.seats.some((seat) => seat.seatId === pending.responderSeatId && seat.alive)) return pending;
  const nextSeatId = shiftNextResponder(pending);
  if (!nextSeatId) return undefined;
  return { ...pending, responderSeatId: nextSeatId };
}

function requireTarget(state: GameState, current: SeatState, targetSeatId?: string): SeatState {
...
function requireReachableTarget(state: GameState, current: SeatState, targetSeatId: string | undefined, card: GameCard): SeatState {
...
function requireTargetWithCards(state: GameState, current: SeatState, targetSeatId: string | undefined, cardName: string): SeatState {
  const target = requireTarget(state, current, targetSeatId);
...
function requireSeatById(state: GameState, seatId: string): SeatState {
  const seat = state.seats.find((item) => item.seatId === seatId);
  if (!seat) throw new Error("座位不存在。");
  return seat;
}

function aliveSeatIds(state: GameState): string[] {
  return state.seats.filter((seat) => seat.alive).map((seat) => seat.seatId);
}

function aliveOtherSeatIds(state: GameState, seatId: string): string[] {
  return state.seats.filter((seat) => seat.alive && seat.seatId !== seatId).map((seat) => seat.seatId);
}

function responseQueueFrom(state: GameState, seatId: string, includeStart = false): string[] {
  ...
  for (let offset = includeStart ? 0 : 1; offset < state.seats.length + (includeStart ? 0 : 1); offset += 1) {
    ...
    if (seat?.alive) result.push(seat.seatId);
  }
  return [...new Set(result)];
}

function findNextAliveSeatIndex(state: GameState): number {
  for (let offset = 1; offset <= state.seats.length; offset += 1) {
    const index = (state.currentSeatIndex + offset) % state.seats.length;
    if (state.seats[index]?.alive) return index;
  }
  return state.currentSeatIndex;
}

function nextAliveSeatAfter(state: GameState, seatId: string): SeatState | undefined {
  const start = state.seats.findIndex((seat) => seat.seatId === seatId);
  for (let offset = 1; offset <= state.seats.length; offset += 1) {
    const seat = state.seats[(start + offset) % state.seats.length];
...
function getSeatDistance(state: GameState, source: SeatState, target: SeatState): number {
...
function getAttackRange(seat: SeatState): number {
...
function hasWeaponKey(seat: SeatState, key: EquipmentKey): boolean {
  return (
    seat.equipment.weapon?.equipmentKey === key ||
    getSkillCards(seat.skillState?.yangExtraWeapons).some((card) => card.equipmentKey === key)
  );
}

function hasArmorKey(seat: SeatState, key: EquipmentKey): boolean {
  return (
    seat.equipment.armor?.equipmentKey === key ||
    getSkillCards(seat.skillState?.yangExtraArmors).some((card) => card.equipmentKey === key)
  );
}

function canReachShaTarget(state: GameState, source: SeatState, target: SeatState): boolean {
  return getSeatDistance(state, source, target) <= getAttackRange(source);
}

function isHongAccompliceBlocked(state: GameState, source: SeatState, target: SeatState): boolean {
  if (!isHongXiliang(target)) return false;
  const round = getRoundNumber(state);
  return (
    source.skillState?.hongAccompliceSourceSeatId === target.seatId &&
    Number(source.skillState?.hongAccompliceRound ?? -1) === round
  );
}

function getEffectiveMaxHp(character: CharacterDefinition): number {
  return character.maxHp + character.skills.reduce((sum, skill) => {
    if (skill.type !== "max_hp_bonus") return sum;
    return sum + Number(skill.params.amount ?? 0);
  }, 0);
}

function getDrawBonus(character: CharacterDefinition): number {
  return character.skills.reduce((sum, skill) => {
    if (skill.type !== "draw_bonus_on_turn_start") return sum;
    return sum + Number(skill.params.amount ?? 0);
  }, 0);
}

function hasAliveFaction(state: GameState, faction: CharacterDefinition["faction"]): boolean {
  return state.seats.some((seat) => seat.alive && seat.character.faction === faction);
}

function applyKgMarker(_state: GameState, source: SeatState, target: SeatState, events: string[]): void {
  target.skillState = {
    ...target.skillState,
...
function triggerKgCardUse(state: GameState, user: SeatState, card: GameCard, events: string[]): void {
...
function triggerJuBoardMarkOnTrick(
  _state: GameState,
  source: SeatState,
  card: GameCard,
  effect: PendingEffect,
  ...
  if (!isJuHui(source) || card.category !== "trick") return;
  const targetIds = new Set<string>();
  const effectData = effect as PendingEffect & {
    targetSeatId?: string;
    targetSeatIds?: string[];
    queue?: string[];
    weaponSeatId?: string;
  ...
  if (effectData.targetSeatId) targetIds.add(effectData.targetSeatId);
  if (effectData.weaponSeatId) targetIds.add(effectData.weaponSeatId);
  if (effectData.victimSeatId) targetIds.add(effectData.victimSeatId);
  for (const seatId of effectData.targetSeatIds ?? []) targetIds.add(seatId);
  for (const seatId of effectData.queue ?? []) targetIds.add(seatId);
...
function prepareBaoFirstTrick(seat: SeatState, card: GameCard, events: string[]): GameCard {
...
function equipCard(state: GameState, seat: SeatState, card: GameCard, events: string[]): void {
  const slot = card.equipmentSlot;
  if (!slot) throw new Error("装备牌缺少装备槽。");
...
function equipCardDefault(state: GameState, seat: SeatState, card: GameCard, events: string[], slot: EquipmentSlot): void {
  if (!slot) throw new Error("装备牌缺少装备槽。");
...
function removeEquipmentCard(state: GameState, seat: SeatState, slot: EquipmentSlot, events: string[]): GameCard | undefined {
...
function detachEquipmentCard(seat: SeatState, slot: EquipmentSlot, events: string[]): GameCard | undefined {
  const card = seat.equipment[slot];
  if (!card) return undefined;
  delete seat.equipment[slot];
  onEquipmentRemoved(seat, card, events);
...
function onEquipmentAdded(seat: SeatState, _card: GameCard, events: string[]): void {
  if (!isWuMao(seat)) return;
  seat.maxHp += 1;
...
function onEquipmentRemoved(seat: SeatState, card: GameCard, events: string[]): void {
  if (!isWuMao(seat)) return;
...
function transferWeaponForJiedao(state: GameState, effect: Extract<PendingEffect, { kind: "jiedao" }>, events: string[]): void {
  const source = requireSeatById(state, effect.sourceSeatId);
  const weaponSeat = requireSeatById(state, effect.weaponSeatId);
...
function collectTargetCardChoices(target: SeatState): ChoiceCardOption[] {
  const hand = target.hand.map((card, index) => choiceFromCard(card, "hand", target.seatId, undefined, `手牌 ${index + 1}`));
  const equipment = Object.entries(target.equipment).flatMap(([slot, card]) =>
    card ? [choiceFromCard(card, "equipment", target.seatId, slot as EquipmentSlot)] : []
  );
  const judge = target.judgementArea.map((card) => choiceFromCard(card, "judge", target.seatId));
  return [...hand, ...equipment, ...judge];
}

function choiceFromCard(card: GameCard, area: CardArea, ownerSeatId?: string, slot?: EquipmentSlot, displayName?: string): ChoiceCardOption {
  return {
...
function requireChoiceOption(pending: PendingChoice, cardId: string, choiceIdValue?: string): ChoiceCardOption {
  const choice = pending.choices.find((item) => item.id === choiceIdValue || item.cardId === cardId);
  if (!choice) throw new Error("选择项不存在。");
  return choice;
}

function takeChoiceCard(state: GameState, choice: ChoiceCardOption): GameCard | undefined {
  if (choice.area === "public") return takeCard(state.publicCards, choice.cardId);
  if (!choice.ownerSeatId) return undefined;
  const owner = requireSeatById(state, choice.ownerSeatId);
  if (choice.area === "hand") return takeCard(owner.hand, choice.cardId);
  if (choice.area === "judge") return takeCard(owner.judgementArea, choice.cardId);
...
function takeCard(cards: GameCard[], cardId: string): GameCard | undefined {
  const index = cards.findIndex((card) => card.id === cardId);
  if (index < 0) return undefined;
  return cards.splice(index, 1)[0];
}

function takeDeterministicHandCard(state: GameState, seat: SeatState, seed: string): GameCard | undefined {
  if (seat.hand.length === 0) return undefined;
...
function discardUsedCard(state: GameState, seat: SeatState, card: GameCard): void {
  discardToPile(state, seat, card);
}

function discardToPile(state: GameState, seat: SeatState, card: GameCard): void {
  seat.discardPile.push(card);
  state.discardPile.push(card);
  state.revealedCards.push(card);
}

function isSha(card: GameCard): boolean {
  return card.cardKey === "sha" || card.cardKey === "fire_sha" || card.cardKey === "thunder_sha";
}

function isShaKey(cardKey: CardKey): boolean {
...
function isBlackSuit(suit: CardSuit): boolean {
  return suit === "spade" || suit === "club";
}

function cardMatchesResponse(card: GameCard, responseType: CardResponseType): boolean {
  if (responseType === "sha") return isSha(card);
...
function finalizeAction(result: GameActionResult): GameActionResult {
...
function flushDeferredCharacterChoice(state: GameState, events: string[]): void {
  if (state.pendingChoice || state.pendingResponse || state.pendingDying || state.phase === "finished") return;
  const round = getRoundNumber(state);
  const sanshui = state.seats.find(
    (seat) =>
      seat.alive &&
      isSanshui(seat) &&
...
function syncActionTimer(state: GameState): GameState {
  ...
  if (state.pendingResponse) {
    const durationSeconds =
      state.pendingResponse.responseType === "wuxie" && state.pendingResponse.mode === "global"
        ? 5
...
function setActionTimer(
  ...
  if (state.actionTimer?.scopeId === scopeId && state.actionTimer.kind === kind && state.actionTimer.seatId === seatId) return;
  state.actionTimer = { kind, seatId, scopeId, durationSeconds, startedAt: Date.now() };
}

function markCardVoice(
  state: GameState,
  seat: SeatState,
  card: GameCard,
  options: { targetSeatId?: string; targetSeatIds?: string[] } = {}
): void {
...
type SkillVoiceVariant = NonNullable<GameState["lastSkillVoice"]>["variant"];

function markSkillVoice(
  state: GameState,
  seat: SeatState,
...
function skillVoiceVariant(skillId: string): SkillVoiceVariant {
  if (skillId.includes("zhangba") || skillId.includes("sha") || skillId.includes("sword") || skillId.includes("blade")) return "slash";
  if (skillId.includes("huo") || skillId.includes("fire")) return "fire";
  if (skillId.includes("lei") || skillId.includes("thunder")) return "thunder";
  if (skillId.includes("tao") || skillId.includes("mercy") || skillId.includes("heal")) return "heal";
  if (skillId.includes("shield") || skillId.includes("dang") || skillId.includes("shenggen") || skillId.includes("copy")) return "buff";
  if (skillId.includes("poison") || skillId.includes("cjj") || skillId.includes("silence") || skillId.includes("konghuang")) return "poison";
  if (skillId.includes("wuxie") || skillId.includes("negate")) return "negate";
  return "trick";
}

function skillVoiceName(skillId: string): string {
...
function normalizeTimerSettings(input?: Partial<GameTimerSettings>): GameTimerSettings {
  return {
...
export function getGameCardCatalog(): GameCard[] {
  return createStarterDeck().filter((card, index, cards) => cards.findIndex((item) => item.name === card.name) === index);
}

function shuffleDeck(cards: GameCard[], seed: string): GameCard[] {
